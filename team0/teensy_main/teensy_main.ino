...
void setup() {
  ...
  initializeIMU(&RateCalibrationRoll, &RateCalibrationPitch, &RateCalibrationYaw);
  initializeBMP280();
  initializeGPS();
  initializeServo();
...
void initializeIMU(float* RateCalibrationRoll, float* RateCalibrationPitch, float* RateCalibrationYaw) {
  ...
  for (int i=0; i<2000; i ++) {
    ...
    readGyroNoCal(&RateRoll, &RatePitch, &RateYaw);
...
void readGyroNoCal(float* gyrox, float* gyroy, float* gyroz) {
  Wire.beginTransmission(0x68);
  Wire.write(0x1A);
  Wire.write(0x05);
  Wire.endTransmission();
